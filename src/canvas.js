...
window.addEventListener('load', function() {
  ...
  let scale = Math.min((canvas.height - document.getElementById("controls-card").clientHeight - 2 * padding) / cylinder.height, (canvas.width - 2 * padding) / cylinder.width);
  let transform = {
    s: scale,
    tx: document.body.clientWidth / 2,
    ty: document.getElementById("controls-card").clientHeight + 1 * padding + cylinder.height * scale / 2
  }
