...
import {MDCChipSet, MDCChipFoundation} from '@material/chips';
...
tabBar.listen('MDCTabBar:activated', (e) => {
    for (let i = 0; i < tabBar.tabList_.length; i++) {
        if (e.detail.index == i) {
...
Array.from(document.getElementsByClassName('var-slider')).forEach(element => {
    ...
    slider.listen('MDCSlider:input', () => {
        ...
        if (options.lineChart) {
            addData(variable, locked, {x:varSliders[variable].value, y:varSliders[locked].value});
        }
    });
});
...
variables.addEventListener('set', (evt) => {
    ...
    if (evt.variable.startsWith('species') || evt.variable.startsWith('moles')) {
        ...
        let speciesIcon = document.getElementById('species-list').children[n].querySelector('span');
        ...
        if (evt.variable.startsWith('species')) {
            ...
            if (document.querySelector(`.species${n} .mdc-chip-set .mdc-chip--selected`)) {
                Array.from(document.querySelectorAll(`.species${n} .mdc-chip-set .mdc-chip--selected`)).forEach(el => {
                    el.classList.remove("mdc-chip--selected");
                });
            }

            // select chosen chip
            let chipEl = document.querySelector(`.species${n} .mdc-chip-set #species-chip-${variables[`species${n}`].toLowerCase()}`)
        ...
        if (iconButton.on) {
            for (let element in toggleLockButtons) {
                if (toggleLockButtons[element] == iconButton) {
                    ...
                    iconButton.on = true;
                    varSliders[element].disabled = true;
                } else {
                    toggleLockButtons[element].root_.removeAttribute('disabled');
                    toggleLockButtons[element].on = false;
                    varSliders[element].disabled = false;
                }
            }
        }
    });
...
Array.from(document.getElementsByClassName('mdc-chip-set')).forEach(chipSetEl => {
    new MDCChipSet(chipSetEl);
    Array.from(chipSetEl.children).forEach(chipEl => {
        chipEl.addEventListener('click', (evt) => {
            // update variables.speciesX
            variables["species" + currentSpecies] = evt.target.innerText.trim();
        });
    });
});
...
trailSwitchEl.addEventListener('click', () => {
    options.trail = trailSwitch.checked;
});
